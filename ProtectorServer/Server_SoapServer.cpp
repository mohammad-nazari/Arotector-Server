/* Server_SoapServer.cpp
   Generated by gSOAP 2.8.27 from F:\Projects\WebService\GeneratedCode\Protector\source\WebService\Protector.h

gSOAP XML Web services tools
Copyright (C) 2000-2015, Robert van Engelen, Genivia Inc. All Rights Reserved.
The soapcpp2 tool and its generated software are released under the GPL.
This program is released under the GPL with the additional exemption that
compiling, linking, and/or using OpenSSL is allowed.
--------------------------------------------------------------------------------
A commercial use license is available from Genivia Inc., contact@genivia.com
--------------------------------------------------------------------------------
*/

#if defined(__BORLANDC__)
#pragma option push -w-8060
#pragma option push -w-8004
#endif
#include "Server_SoapH.h"

SOAP_SOURCE_STAMP("@(#) Server_SoapServer.cpp ver 2.8.27 2016-03-29 20:01:15 GMT")


extern "C" SOAP_FMAC5 int SOAP_FMAC6 soap_serve(struct soap *soap)
{
#ifndef WITH_FASTCGI
	unsigned int k = soap->max_keep_alive;
#endif
	do
	{
#ifndef WITH_FASTCGI
		if (soap->max_keep_alive > 0 && !--k)
			soap->keep_alive = 0;
#endif
		if (soap_begin_serve(soap))
		{	if (soap->error >= SOAP_STOP)
				continue;
			return soap->error;
		}
		if (soap_serve_request(soap) || (soap->fserveloop && soap->fserveloop(soap)))
		{
#ifdef WITH_FASTCGI
			soap_send_fault(soap);
#else
			return soap_send_fault(soap);
#endif
		}

#ifdef WITH_FASTCGI
		soap_destroy(soap);
		soap_end(soap);
	} while (1);
#else
	} while (soap->keep_alive);
#endif
	return SOAP_OK;
}

#ifndef WITH_NOSERVEREQUEST
extern "C" SOAP_FMAC5 int SOAP_FMAC6 soap_serve_request(struct soap *soap)
{
	soap_peek_element(soap);
	if (!soap_match_tag(soap, soap->tag, "ns:GetDeviceStatus"))
		return soap_serve_ns__GetDeviceStatus(soap);
	if (!soap_match_tag(soap, soap->tag, "ns:GetAllDeviceStatus"))
		return soap_serve_ns__GetAllDeviceStatus(soap);
	if (!soap_match_tag(soap, soap->tag, "ns:SetDeviceSetting"))
		return soap_serve_ns__SetDeviceSetting(soap);
	if (!soap_match_tag(soap, soap->tag, "ns:SetAllDeviceSetting"))
		return soap_serve_ns__SetAllDeviceSetting(soap);
	if (!soap_match_tag(soap, soap->tag, "ns:DeleteDevices"))
		return soap_serve_ns__DeleteDevices(soap);
	if (!soap_match_tag(soap, soap->tag, "ns:Login"))
		return soap_serve_ns__Login(soap);
	if (!soap_match_tag(soap, soap->tag, "ns:GetUserDeviceList"))
		return soap_serve_ns__GetUserDeviceList(soap);
	if (!soap_match_tag(soap, soap->tag, "ns:GetAllDeviceList"))
		return soap_serve_ns__GetAllDeviceList(soap);
	if (!soap_match_tag(soap, soap->tag, "ns:GetUsersList"))
		return soap_serve_ns__GetUsersList(soap);
	if (!soap_match_tag(soap, soap->tag, "ns:AddUser"))
		return soap_serve_ns__AddUser(soap);
	if (!soap_match_tag(soap, soap->tag, "ns:UpdateUser"))
		return soap_serve_ns__UpdateUser(soap);
	if (!soap_match_tag(soap, soap->tag, "ns:DeleteUser"))
		return soap_serve_ns__DeleteUser(soap);
	if (!soap_match_tag(soap, soap->tag, "ns:AssignDeviceToUser"))
		return soap_serve_ns__AssignDeviceToUser(soap);
	if (!soap_match_tag(soap, soap->tag, "ns:DeleteDeviceFromUserList"))
		return soap_serve_ns__DeleteDeviceFromUserList(soap);
	if (!soap_match_tag(soap, soap->tag, "ns:GetUserDevices"))
		return soap_serve_ns__GetUserDevices(soap);
	if (!soap_match_tag(soap, soap->tag, "ns:SaveActivityLog"))
		return soap_serve_ns__SaveActivityLog(soap);
	if (!soap_match_tag(soap, soap->tag, "ns:GetActivityLog"))
		return soap_serve_ns__GetActivityLog(soap);
	if (!soap_match_tag(soap, soap->tag, "ns:SaveUserLog"))
		return soap_serve_ns__SaveUserLog(soap);
	if (!soap_match_tag(soap, soap->tag, "ns:GetUserLog"))
		return soap_serve_ns__GetUserLog(soap);
	if (!soap_match_tag(soap, soap->tag, "ns:SaveDeviceStatusLog"))
		return soap_serve_ns__SaveDeviceStatusLog(soap);
	if (!soap_match_tag(soap, soap->tag, "ns:GetDeviceStatusLog"))
		return soap_serve_ns__GetDeviceStatusLog(soap);
	if (!soap_match_tag(soap, soap->tag, "ns:GetServerOptions"))
		return soap_serve_ns__GetServerOptions(soap);
	if (!soap_match_tag(soap, soap->tag, "ns:SetServerOptions"))
		return soap_serve_ns__SetServerOptions(soap);
	if (!soap_match_tag(soap, soap->tag, "ns:GetCityAndLocations"))
		return soap_serve_ns__GetCityAndLocations(soap);
	if (!soap_match_tag(soap, soap->tag, "ns:ChangeCityName"))
		return soap_serve_ns__ChangeCityName(soap);
	if (!soap_match_tag(soap, soap->tag, "ns:ChangeLocationName"))
		return soap_serve_ns__ChangeLocationName(soap);
	if (!soap_match_tag(soap, soap->tag, "ns:ChangeDeviceName"))
		return soap_serve_ns__ChangeDeviceName(soap);
	if (!soap_match_tag(soap, soap->tag, "ns:TestWorld"))
		return soap_serve_ns__TestWorld(soap);
	return soap->error = SOAP_NO_METHOD;
}
#endif

SOAP_FMAC5 int SOAP_FMAC6 soap_serve_ns__GetDeviceStatus(struct soap *soap)
{	struct ns__GetDeviceStatus soap_tmp_ns__GetDeviceStatus;
	ns__Device responseDeviceInfo;
	responseDeviceInfo.soap_default(soap);
	soap_default_ns__GetDeviceStatus(soap, &soap_tmp_ns__GetDeviceStatus);
	if (!soap_get_ns__GetDeviceStatus(soap, &soap_tmp_ns__GetDeviceStatus, "ns:GetDeviceStatus", NULL))
		return soap->error;
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
		return soap->error;
	soap->error = ns__GetDeviceStatus(soap, soap_tmp_ns__GetDeviceStatus.requestUserInfo, soap_tmp_ns__GetDeviceStatus.requestDeviceInfo, responseDeviceInfo);
	if (soap->error)
		return soap->error;
	soap->encodingStyle = NULL;
	soap_serializeheader(soap);
	responseDeviceInfo.soap_serialize(soap);
	if (soap_begin_count(soap))
		return soap->error;
	if (soap->mode & SOAP_IO_LENGTH)
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || responseDeviceInfo.soap_put(soap, "ns:Device", "")
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	};
	if (soap_end_count(soap)
	 || soap_response(soap, SOAP_OK)
	 || soap_envelope_begin_out(soap)
	 || soap_putheader(soap)
	 || soap_body_begin_out(soap)
	 || responseDeviceInfo.soap_put(soap, "ns:Device", "")
	 || soap_body_end_out(soap)
	 || soap_envelope_end_out(soap)
	 || soap_end_send(soap))
		return soap->error;
	return soap_closesock(soap);
}

SOAP_FMAC5 int SOAP_FMAC6 soap_serve_ns__GetAllDeviceStatus(struct soap *soap)
{	struct ns__GetAllDeviceStatus soap_tmp_ns__GetAllDeviceStatus;
	ns__UserDevice responseUserDevicesInfo;
	responseUserDevicesInfo.soap_default(soap);
	soap_default_ns__GetAllDeviceStatus(soap, &soap_tmp_ns__GetAllDeviceStatus);
	if (!soap_get_ns__GetAllDeviceStatus(soap, &soap_tmp_ns__GetAllDeviceStatus, "ns:GetAllDeviceStatus", NULL))
		return soap->error;
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
		return soap->error;
	soap->error = ns__GetAllDeviceStatus(soap, soap_tmp_ns__GetAllDeviceStatus.requestUserInfo, responseUserDevicesInfo);
	if (soap->error)
		return soap->error;
	soap->encodingStyle = NULL;
	soap_serializeheader(soap);
	responseUserDevicesInfo.soap_serialize(soap);
	if (soap_begin_count(soap))
		return soap->error;
	if (soap->mode & SOAP_IO_LENGTH)
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || responseUserDevicesInfo.soap_put(soap, "ns:UserDevice", "")
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	};
	if (soap_end_count(soap)
	 || soap_response(soap, SOAP_OK)
	 || soap_envelope_begin_out(soap)
	 || soap_putheader(soap)
	 || soap_body_begin_out(soap)
	 || responseUserDevicesInfo.soap_put(soap, "ns:UserDevice", "")
	 || soap_body_end_out(soap)
	 || soap_envelope_end_out(soap)
	 || soap_end_send(soap))
		return soap->error;
	return soap_closesock(soap);
}

SOAP_FMAC5 int SOAP_FMAC6 soap_serve_ns__SetDeviceSetting(struct soap *soap)
{	struct ns__SetDeviceSetting soap_tmp_ns__SetDeviceSetting;
	ns__ErrorCode responseErrorInfo;
	responseErrorInfo.soap_default(soap);
	soap_default_ns__SetDeviceSetting(soap, &soap_tmp_ns__SetDeviceSetting);
	if (!soap_get_ns__SetDeviceSetting(soap, &soap_tmp_ns__SetDeviceSetting, "ns:SetDeviceSetting", NULL))
		return soap->error;
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
		return soap->error;
	soap->error = ns__SetDeviceSetting(soap, soap_tmp_ns__SetDeviceSetting.requestUserInfo, soap_tmp_ns__SetDeviceSetting.requestDeviceInfo, responseErrorInfo);
	if (soap->error)
		return soap->error;
	soap->encodingStyle = NULL;
	soap_serializeheader(soap);
	responseErrorInfo.soap_serialize(soap);
	if (soap_begin_count(soap))
		return soap->error;
	if (soap->mode & SOAP_IO_LENGTH)
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || responseErrorInfo.soap_put(soap, "ns:ErrorCode", "")
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	};
	if (soap_end_count(soap)
	 || soap_response(soap, SOAP_OK)
	 || soap_envelope_begin_out(soap)
	 || soap_putheader(soap)
	 || soap_body_begin_out(soap)
	 || responseErrorInfo.soap_put(soap, "ns:ErrorCode", "")
	 || soap_body_end_out(soap)
	 || soap_envelope_end_out(soap)
	 || soap_end_send(soap))
		return soap->error;
	return soap_closesock(soap);
}

SOAP_FMAC5 int SOAP_FMAC6 soap_serve_ns__SetAllDeviceSetting(struct soap *soap)
{	struct ns__SetAllDeviceSetting soap_tmp_ns__SetAllDeviceSetting;
	struct ns__SetAllDeviceSettingResponse soap_tmp_ns__SetAllDeviceSettingResponse;
	soap_default_ns__SetAllDeviceSettingResponse(soap, &soap_tmp_ns__SetAllDeviceSettingResponse);
	soap_default_ns__SetAllDeviceSetting(soap, &soap_tmp_ns__SetAllDeviceSetting);
	if (!soap_get_ns__SetAllDeviceSetting(soap, &soap_tmp_ns__SetAllDeviceSetting, "ns:SetAllDeviceSetting", NULL))
		return soap->error;
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
		return soap->error;
	soap->error = ns__SetAllDeviceSetting(soap, soap_tmp_ns__SetAllDeviceSetting.requestUserInfo, soap_tmp_ns__SetAllDeviceSetting.requestDeviceInfo, soap_tmp_ns__SetAllDeviceSettingResponse.responseErrorInfo);
	if (soap->error)
		return soap->error;
	soap->encodingStyle = NULL;
	soap_serializeheader(soap);
	soap_serialize_ns__SetAllDeviceSettingResponse(soap, &soap_tmp_ns__SetAllDeviceSettingResponse);
	if (soap_begin_count(soap))
		return soap->error;
	if (soap->mode & SOAP_IO_LENGTH)
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || soap_put_ns__SetAllDeviceSettingResponse(soap, &soap_tmp_ns__SetAllDeviceSettingResponse, "ns:SetAllDeviceSettingResponse", NULL)
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	};
	if (soap_end_count(soap)
	 || soap_response(soap, SOAP_OK)
	 || soap_envelope_begin_out(soap)
	 || soap_putheader(soap)
	 || soap_body_begin_out(soap)
	 || soap_put_ns__SetAllDeviceSettingResponse(soap, &soap_tmp_ns__SetAllDeviceSettingResponse, "ns:SetAllDeviceSettingResponse", NULL)
	 || soap_body_end_out(soap)
	 || soap_envelope_end_out(soap)
	 || soap_end_send(soap))
		return soap->error;
	return soap_closesock(soap);
}

SOAP_FMAC5 int SOAP_FMAC6 soap_serve_ns__DeleteDevices(struct soap *soap)
{	struct ns__DeleteDevices soap_tmp_ns__DeleteDevices;
	ns__ErrorCode responseErrorInfo;
	responseErrorInfo.soap_default(soap);
	soap_default_ns__DeleteDevices(soap, &soap_tmp_ns__DeleteDevices);
	if (!soap_get_ns__DeleteDevices(soap, &soap_tmp_ns__DeleteDevices, "ns:DeleteDevices", NULL))
		return soap->error;
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
		return soap->error;
	soap->error = ns__DeleteDevices(soap, soap_tmp_ns__DeleteDevices.requestUserInfo, soap_tmp_ns__DeleteDevices.requestDeviceList, responseErrorInfo);
	if (soap->error)
		return soap->error;
	soap->encodingStyle = NULL;
	soap_serializeheader(soap);
	responseErrorInfo.soap_serialize(soap);
	if (soap_begin_count(soap))
		return soap->error;
	if (soap->mode & SOAP_IO_LENGTH)
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || responseErrorInfo.soap_put(soap, "ns:ErrorCode", "")
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	};
	if (soap_end_count(soap)
	 || soap_response(soap, SOAP_OK)
	 || soap_envelope_begin_out(soap)
	 || soap_putheader(soap)
	 || soap_body_begin_out(soap)
	 || responseErrorInfo.soap_put(soap, "ns:ErrorCode", "")
	 || soap_body_end_out(soap)
	 || soap_envelope_end_out(soap)
	 || soap_end_send(soap))
		return soap->error;
	return soap_closesock(soap);
}

SOAP_FMAC5 int SOAP_FMAC6 soap_serve_ns__Login(struct soap *soap)
{	struct ns__Login soap_tmp_ns__Login;
	ns__User responseUserInfo;
	responseUserInfo.soap_default(soap);
	soap_default_ns__Login(soap, &soap_tmp_ns__Login);
	if (!soap_get_ns__Login(soap, &soap_tmp_ns__Login, "ns:Login", NULL))
		return soap->error;
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
		return soap->error;
	soap->error = ns__Login(soap, soap_tmp_ns__Login.requestUserInfo, responseUserInfo);
	if (soap->error)
		return soap->error;
	soap->encodingStyle = NULL;
	soap_serializeheader(soap);
	responseUserInfo.soap_serialize(soap);
	if (soap_begin_count(soap))
		return soap->error;
	if (soap->mode & SOAP_IO_LENGTH)
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || responseUserInfo.soap_put(soap, "ns:User", "")
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	};
	if (soap_end_count(soap)
	 || soap_response(soap, SOAP_OK)
	 || soap_envelope_begin_out(soap)
	 || soap_putheader(soap)
	 || soap_body_begin_out(soap)
	 || responseUserInfo.soap_put(soap, "ns:User", "")
	 || soap_body_end_out(soap)
	 || soap_envelope_end_out(soap)
	 || soap_end_send(soap))
		return soap->error;
	return soap_closesock(soap);
}

SOAP_FMAC5 int SOAP_FMAC6 soap_serve_ns__GetUserDeviceList(struct soap *soap)
{	struct ns__GetUserDeviceList soap_tmp_ns__GetUserDeviceList;
	ns__UserDevice responseUserDevicesInfo;
	responseUserDevicesInfo.soap_default(soap);
	soap_default_ns__GetUserDeviceList(soap, &soap_tmp_ns__GetUserDeviceList);
	if (!soap_get_ns__GetUserDeviceList(soap, &soap_tmp_ns__GetUserDeviceList, "ns:GetUserDeviceList", NULL))
		return soap->error;
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
		return soap->error;
	soap->error = ns__GetUserDeviceList(soap, soap_tmp_ns__GetUserDeviceList.requestUserInfo, responseUserDevicesInfo);
	if (soap->error)
		return soap->error;
	soap->encodingStyle = NULL;
	soap_serializeheader(soap);
	responseUserDevicesInfo.soap_serialize(soap);
	if (soap_begin_count(soap))
		return soap->error;
	if (soap->mode & SOAP_IO_LENGTH)
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || responseUserDevicesInfo.soap_put(soap, "ns:UserDevice", "")
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	};
	if (soap_end_count(soap)
	 || soap_response(soap, SOAP_OK)
	 || soap_envelope_begin_out(soap)
	 || soap_putheader(soap)
	 || soap_body_begin_out(soap)
	 || responseUserDevicesInfo.soap_put(soap, "ns:UserDevice", "")
	 || soap_body_end_out(soap)
	 || soap_envelope_end_out(soap)
	 || soap_end_send(soap))
		return soap->error;
	return soap_closesock(soap);
}

SOAP_FMAC5 int SOAP_FMAC6 soap_serve_ns__GetAllDeviceList(struct soap *soap)
{	struct ns__GetAllDeviceList soap_tmp_ns__GetAllDeviceList;
	ns__UserDevice responseUserDevicesInfo;
	responseUserDevicesInfo.soap_default(soap);
	soap_default_ns__GetAllDeviceList(soap, &soap_tmp_ns__GetAllDeviceList);
	if (!soap_get_ns__GetAllDeviceList(soap, &soap_tmp_ns__GetAllDeviceList, "ns:GetAllDeviceList", NULL))
		return soap->error;
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
		return soap->error;
	soap->error = ns__GetAllDeviceList(soap, soap_tmp_ns__GetAllDeviceList.requestUserInfo, responseUserDevicesInfo);
	if (soap->error)
		return soap->error;
	soap->encodingStyle = NULL;
	soap_serializeheader(soap);
	responseUserDevicesInfo.soap_serialize(soap);
	if (soap_begin_count(soap))
		return soap->error;
	if (soap->mode & SOAP_IO_LENGTH)
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || responseUserDevicesInfo.soap_put(soap, "ns:UserDevice", "")
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	};
	if (soap_end_count(soap)
	 || soap_response(soap, SOAP_OK)
	 || soap_envelope_begin_out(soap)
	 || soap_putheader(soap)
	 || soap_body_begin_out(soap)
	 || responseUserDevicesInfo.soap_put(soap, "ns:UserDevice", "")
	 || soap_body_end_out(soap)
	 || soap_envelope_end_out(soap)
	 || soap_end_send(soap))
		return soap->error;
	return soap_closesock(soap);
}

SOAP_FMAC5 int SOAP_FMAC6 soap_serve_ns__GetUsersList(struct soap *soap)
{	struct ns__GetUsersList soap_tmp_ns__GetUsersList;
	ns__UsersList responseUsersList;
	responseUsersList.soap_default(soap);
	soap_default_ns__GetUsersList(soap, &soap_tmp_ns__GetUsersList);
	if (!soap_get_ns__GetUsersList(soap, &soap_tmp_ns__GetUsersList, "ns:GetUsersList", NULL))
		return soap->error;
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
		return soap->error;
	soap->error = ns__GetUsersList(soap, soap_tmp_ns__GetUsersList.requestUserInfo, responseUsersList);
	if (soap->error)
		return soap->error;
	soap->encodingStyle = NULL;
	soap_serializeheader(soap);
	responseUsersList.soap_serialize(soap);
	if (soap_begin_count(soap))
		return soap->error;
	if (soap->mode & SOAP_IO_LENGTH)
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || responseUsersList.soap_put(soap, "ns:UsersList", "")
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	};
	if (soap_end_count(soap)
	 || soap_response(soap, SOAP_OK)
	 || soap_envelope_begin_out(soap)
	 || soap_putheader(soap)
	 || soap_body_begin_out(soap)
	 || responseUsersList.soap_put(soap, "ns:UsersList", "")
	 || soap_body_end_out(soap)
	 || soap_envelope_end_out(soap)
	 || soap_end_send(soap))
		return soap->error;
	return soap_closesock(soap);
}

SOAP_FMAC5 int SOAP_FMAC6 soap_serve_ns__AddUser(struct soap *soap)
{	struct ns__AddUser soap_tmp_ns__AddUser;
	ns__ErrorCode responseErrorInfo;
	responseErrorInfo.soap_default(soap);
	soap_default_ns__AddUser(soap, &soap_tmp_ns__AddUser);
	if (!soap_get_ns__AddUser(soap, &soap_tmp_ns__AddUser, "ns:AddUser", NULL))
		return soap->error;
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
		return soap->error;
	soap->error = ns__AddUser(soap, soap_tmp_ns__AddUser.requestUserInfo, soap_tmp_ns__AddUser.requestNewUserInfo, responseErrorInfo);
	if (soap->error)
		return soap->error;
	soap->encodingStyle = NULL;
	soap_serializeheader(soap);
	responseErrorInfo.soap_serialize(soap);
	if (soap_begin_count(soap))
		return soap->error;
	if (soap->mode & SOAP_IO_LENGTH)
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || responseErrorInfo.soap_put(soap, "ns:ErrorCode", "")
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	};
	if (soap_end_count(soap)
	 || soap_response(soap, SOAP_OK)
	 || soap_envelope_begin_out(soap)
	 || soap_putheader(soap)
	 || soap_body_begin_out(soap)
	 || responseErrorInfo.soap_put(soap, "ns:ErrorCode", "")
	 || soap_body_end_out(soap)
	 || soap_envelope_end_out(soap)
	 || soap_end_send(soap))
		return soap->error;
	return soap_closesock(soap);
}

SOAP_FMAC5 int SOAP_FMAC6 soap_serve_ns__UpdateUser(struct soap *soap)
{	struct ns__UpdateUser soap_tmp_ns__UpdateUser;
	ns__ErrorCode responseErrorInfo;
	responseErrorInfo.soap_default(soap);
	soap_default_ns__UpdateUser(soap, &soap_tmp_ns__UpdateUser);
	if (!soap_get_ns__UpdateUser(soap, &soap_tmp_ns__UpdateUser, "ns:UpdateUser", NULL))
		return soap->error;
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
		return soap->error;
	soap->error = ns__UpdateUser(soap, soap_tmp_ns__UpdateUser.requestUserInfo, soap_tmp_ns__UpdateUser.requestNewUserInfo, responseErrorInfo);
	if (soap->error)
		return soap->error;
	soap->encodingStyle = NULL;
	soap_serializeheader(soap);
	responseErrorInfo.soap_serialize(soap);
	if (soap_begin_count(soap))
		return soap->error;
	if (soap->mode & SOAP_IO_LENGTH)
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || responseErrorInfo.soap_put(soap, "ns:ErrorCode", "")
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	};
	if (soap_end_count(soap)
	 || soap_response(soap, SOAP_OK)
	 || soap_envelope_begin_out(soap)
	 || soap_putheader(soap)
	 || soap_body_begin_out(soap)
	 || responseErrorInfo.soap_put(soap, "ns:ErrorCode", "")
	 || soap_body_end_out(soap)
	 || soap_envelope_end_out(soap)
	 || soap_end_send(soap))
		return soap->error;
	return soap_closesock(soap);
}

SOAP_FMAC5 int SOAP_FMAC6 soap_serve_ns__DeleteUser(struct soap *soap)
{	struct ns__DeleteUser soap_tmp_ns__DeleteUser;
	ns__ErrorCode responseErrorInfo;
	responseErrorInfo.soap_default(soap);
	soap_default_ns__DeleteUser(soap, &soap_tmp_ns__DeleteUser);
	if (!soap_get_ns__DeleteUser(soap, &soap_tmp_ns__DeleteUser, "ns:DeleteUser", NULL))
		return soap->error;
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
		return soap->error;
	soap->error = ns__DeleteUser(soap, soap_tmp_ns__DeleteUser.requestUserInfo, soap_tmp_ns__DeleteUser.requestDeleteUserInfo, responseErrorInfo);
	if (soap->error)
		return soap->error;
	soap->encodingStyle = NULL;
	soap_serializeheader(soap);
	responseErrorInfo.soap_serialize(soap);
	if (soap_begin_count(soap))
		return soap->error;
	if (soap->mode & SOAP_IO_LENGTH)
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || responseErrorInfo.soap_put(soap, "ns:ErrorCode", "")
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	};
	if (soap_end_count(soap)
	 || soap_response(soap, SOAP_OK)
	 || soap_envelope_begin_out(soap)
	 || soap_putheader(soap)
	 || soap_body_begin_out(soap)
	 || responseErrorInfo.soap_put(soap, "ns:ErrorCode", "")
	 || soap_body_end_out(soap)
	 || soap_envelope_end_out(soap)
	 || soap_end_send(soap))
		return soap->error;
	return soap_closesock(soap);
}

SOAP_FMAC5 int SOAP_FMAC6 soap_serve_ns__AssignDeviceToUser(struct soap *soap)
{	struct ns__AssignDeviceToUser soap_tmp_ns__AssignDeviceToUser;
	ns__ErrorCode responseErrorInfo;
	responseErrorInfo.soap_default(soap);
	soap_default_ns__AssignDeviceToUser(soap, &soap_tmp_ns__AssignDeviceToUser);
	if (!soap_get_ns__AssignDeviceToUser(soap, &soap_tmp_ns__AssignDeviceToUser, "ns:AssignDeviceToUser", NULL))
		return soap->error;
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
		return soap->error;
	soap->error = ns__AssignDeviceToUser(soap, soap_tmp_ns__AssignDeviceToUser.requestUserInfo, soap_tmp_ns__AssignDeviceToUser.requestUserDeviceInfo, responseErrorInfo);
	if (soap->error)
		return soap->error;
	soap->encodingStyle = NULL;
	soap_serializeheader(soap);
	responseErrorInfo.soap_serialize(soap);
	if (soap_begin_count(soap))
		return soap->error;
	if (soap->mode & SOAP_IO_LENGTH)
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || responseErrorInfo.soap_put(soap, "ns:ErrorCode", "")
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	};
	if (soap_end_count(soap)
	 || soap_response(soap, SOAP_OK)
	 || soap_envelope_begin_out(soap)
	 || soap_putheader(soap)
	 || soap_body_begin_out(soap)
	 || responseErrorInfo.soap_put(soap, "ns:ErrorCode", "")
	 || soap_body_end_out(soap)
	 || soap_envelope_end_out(soap)
	 || soap_end_send(soap))
		return soap->error;
	return soap_closesock(soap);
}

SOAP_FMAC5 int SOAP_FMAC6 soap_serve_ns__DeleteDeviceFromUserList(struct soap *soap)
{	struct ns__DeleteDeviceFromUserList soap_tmp_ns__DeleteDeviceFromUserList;
	ns__ErrorCode responseErrorInfo;
	responseErrorInfo.soap_default(soap);
	soap_default_ns__DeleteDeviceFromUserList(soap, &soap_tmp_ns__DeleteDeviceFromUserList);
	if (!soap_get_ns__DeleteDeviceFromUserList(soap, &soap_tmp_ns__DeleteDeviceFromUserList, "ns:DeleteDeviceFromUserList", NULL))
		return soap->error;
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
		return soap->error;
	soap->error = ns__DeleteDeviceFromUserList(soap, soap_tmp_ns__DeleteDeviceFromUserList.requestUserInfo, soap_tmp_ns__DeleteDeviceFromUserList.requestUserDeviceInfo, responseErrorInfo);
	if (soap->error)
		return soap->error;
	soap->encodingStyle = NULL;
	soap_serializeheader(soap);
	responseErrorInfo.soap_serialize(soap);
	if (soap_begin_count(soap))
		return soap->error;
	if (soap->mode & SOAP_IO_LENGTH)
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || responseErrorInfo.soap_put(soap, "ns:ErrorCode", "")
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	};
	if (soap_end_count(soap)
	 || soap_response(soap, SOAP_OK)
	 || soap_envelope_begin_out(soap)
	 || soap_putheader(soap)
	 || soap_body_begin_out(soap)
	 || responseErrorInfo.soap_put(soap, "ns:ErrorCode", "")
	 || soap_body_end_out(soap)
	 || soap_envelope_end_out(soap)
	 || soap_end_send(soap))
		return soap->error;
	return soap_closesock(soap);
}

SOAP_FMAC5 int SOAP_FMAC6 soap_serve_ns__GetUserDevices(struct soap *soap)
{	struct ns__GetUserDevices soap_tmp_ns__GetUserDevices;
	ns__UserDevice responseUserDevicesInfo;
	responseUserDevicesInfo.soap_default(soap);
	soap_default_ns__GetUserDevices(soap, &soap_tmp_ns__GetUserDevices);
	if (!soap_get_ns__GetUserDevices(soap, &soap_tmp_ns__GetUserDevices, "ns:GetUserDevices", NULL))
		return soap->error;
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
		return soap->error;
	soap->error = ns__GetUserDevices(soap, soap_tmp_ns__GetUserDevices.requestUserInfo, soap_tmp_ns__GetUserDevices.requestSelectedUserInfo, responseUserDevicesInfo);
	if (soap->error)
		return soap->error;
	soap->encodingStyle = NULL;
	soap_serializeheader(soap);
	responseUserDevicesInfo.soap_serialize(soap);
	if (soap_begin_count(soap))
		return soap->error;
	if (soap->mode & SOAP_IO_LENGTH)
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || responseUserDevicesInfo.soap_put(soap, "ns:UserDevice", "")
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	};
	if (soap_end_count(soap)
	 || soap_response(soap, SOAP_OK)
	 || soap_envelope_begin_out(soap)
	 || soap_putheader(soap)
	 || soap_body_begin_out(soap)
	 || responseUserDevicesInfo.soap_put(soap, "ns:UserDevice", "")
	 || soap_body_end_out(soap)
	 || soap_envelope_end_out(soap)
	 || soap_end_send(soap))
		return soap->error;
	return soap_closesock(soap);
}

SOAP_FMAC5 int SOAP_FMAC6 soap_serve_ns__SaveActivityLog(struct soap *soap)
{	struct ns__SaveActivityLog soap_tmp_ns__SaveActivityLog;
	ns__ErrorCode responseErrorInfo;
	responseErrorInfo.soap_default(soap);
	soap_default_ns__SaveActivityLog(soap, &soap_tmp_ns__SaveActivityLog);
	if (!soap_get_ns__SaveActivityLog(soap, &soap_tmp_ns__SaveActivityLog, "ns:SaveActivityLog", NULL))
		return soap->error;
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
		return soap->error;
	soap->error = ns__SaveActivityLog(soap, soap_tmp_ns__SaveActivityLog.requestUserInfo, soap_tmp_ns__SaveActivityLog.requestActivityLogInfo, responseErrorInfo);
	if (soap->error)
		return soap->error;
	soap->encodingStyle = NULL;
	soap_serializeheader(soap);
	responseErrorInfo.soap_serialize(soap);
	if (soap_begin_count(soap))
		return soap->error;
	if (soap->mode & SOAP_IO_LENGTH)
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || responseErrorInfo.soap_put(soap, "ns:ErrorCode", "")
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	};
	if (soap_end_count(soap)
	 || soap_response(soap, SOAP_OK)
	 || soap_envelope_begin_out(soap)
	 || soap_putheader(soap)
	 || soap_body_begin_out(soap)
	 || responseErrorInfo.soap_put(soap, "ns:ErrorCode", "")
	 || soap_body_end_out(soap)
	 || soap_envelope_end_out(soap)
	 || soap_end_send(soap))
		return soap->error;
	return soap_closesock(soap);
}

SOAP_FMAC5 int SOAP_FMAC6 soap_serve_ns__GetActivityLog(struct soap *soap)
{	struct ns__GetActivityLog soap_tmp_ns__GetActivityLog;
	struct ns__GetActivityLogResponse soap_tmp_ns__GetActivityLogResponse;
	soap_default_ns__GetActivityLogResponse(soap, &soap_tmp_ns__GetActivityLogResponse);
	soap_default_ns__GetActivityLog(soap, &soap_tmp_ns__GetActivityLog);
	if (!soap_get_ns__GetActivityLog(soap, &soap_tmp_ns__GetActivityLog, "ns:GetActivityLog", NULL))
		return soap->error;
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
		return soap->error;
	soap->error = ns__GetActivityLog(soap, soap_tmp_ns__GetActivityLog.requestUserInfo, soap_tmp_ns__GetActivityLog.requestReportInfo, soap_tmp_ns__GetActivityLogResponse.responseActivityLogInfo);
	if (soap->error)
		return soap->error;
	soap->encodingStyle = NULL;
	soap_serializeheader(soap);
	soap_serialize_ns__GetActivityLogResponse(soap, &soap_tmp_ns__GetActivityLogResponse);
	if (soap_begin_count(soap))
		return soap->error;
	if (soap->mode & SOAP_IO_LENGTH)
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || soap_put_ns__GetActivityLogResponse(soap, &soap_tmp_ns__GetActivityLogResponse, "ns:GetActivityLogResponse", NULL)
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	};
	if (soap_end_count(soap)
	 || soap_response(soap, SOAP_OK)
	 || soap_envelope_begin_out(soap)
	 || soap_putheader(soap)
	 || soap_body_begin_out(soap)
	 || soap_put_ns__GetActivityLogResponse(soap, &soap_tmp_ns__GetActivityLogResponse, "ns:GetActivityLogResponse", NULL)
	 || soap_body_end_out(soap)
	 || soap_envelope_end_out(soap)
	 || soap_end_send(soap))
		return soap->error;
	return soap_closesock(soap);
}

SOAP_FMAC5 int SOAP_FMAC6 soap_serve_ns__SaveUserLog(struct soap *soap)
{	struct ns__SaveUserLog soap_tmp_ns__SaveUserLog;
	ns__ErrorCode responseErrorInfo;
	responseErrorInfo.soap_default(soap);
	soap_default_ns__SaveUserLog(soap, &soap_tmp_ns__SaveUserLog);
	if (!soap_get_ns__SaveUserLog(soap, &soap_tmp_ns__SaveUserLog, "ns:SaveUserLog", NULL))
		return soap->error;
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
		return soap->error;
	soap->error = ns__SaveUserLog(soap, soap_tmp_ns__SaveUserLog.requestUserInfo, soap_tmp_ns__SaveUserLog.requestActivityLogInfo, responseErrorInfo);
	if (soap->error)
		return soap->error;
	soap->encodingStyle = NULL;
	soap_serializeheader(soap);
	responseErrorInfo.soap_serialize(soap);
	if (soap_begin_count(soap))
		return soap->error;
	if (soap->mode & SOAP_IO_LENGTH)
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || responseErrorInfo.soap_put(soap, "ns:ErrorCode", "")
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	};
	if (soap_end_count(soap)
	 || soap_response(soap, SOAP_OK)
	 || soap_envelope_begin_out(soap)
	 || soap_putheader(soap)
	 || soap_body_begin_out(soap)
	 || responseErrorInfo.soap_put(soap, "ns:ErrorCode", "")
	 || soap_body_end_out(soap)
	 || soap_envelope_end_out(soap)
	 || soap_end_send(soap))
		return soap->error;
	return soap_closesock(soap);
}

SOAP_FMAC5 int SOAP_FMAC6 soap_serve_ns__GetUserLog(struct soap *soap)
{	struct ns__GetUserLog soap_tmp_ns__GetUserLog;
	struct ns__GetUserLogResponse soap_tmp_ns__GetUserLogResponse;
	soap_default_ns__GetUserLogResponse(soap, &soap_tmp_ns__GetUserLogResponse);
	soap_default_ns__GetUserLog(soap, &soap_tmp_ns__GetUserLog);
	if (!soap_get_ns__GetUserLog(soap, &soap_tmp_ns__GetUserLog, "ns:GetUserLog", NULL))
		return soap->error;
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
		return soap->error;
	soap->error = ns__GetUserLog(soap, soap_tmp_ns__GetUserLog.requestUserInfo, soap_tmp_ns__GetUserLog.requestReportInfo, soap_tmp_ns__GetUserLogResponse.responseActivityLogInfo);
	if (soap->error)
		return soap->error;
	soap->encodingStyle = NULL;
	soap_serializeheader(soap);
	soap_serialize_ns__GetUserLogResponse(soap, &soap_tmp_ns__GetUserLogResponse);
	if (soap_begin_count(soap))
		return soap->error;
	if (soap->mode & SOAP_IO_LENGTH)
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || soap_put_ns__GetUserLogResponse(soap, &soap_tmp_ns__GetUserLogResponse, "ns:GetUserLogResponse", NULL)
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	};
	if (soap_end_count(soap)
	 || soap_response(soap, SOAP_OK)
	 || soap_envelope_begin_out(soap)
	 || soap_putheader(soap)
	 || soap_body_begin_out(soap)
	 || soap_put_ns__GetUserLogResponse(soap, &soap_tmp_ns__GetUserLogResponse, "ns:GetUserLogResponse", NULL)
	 || soap_body_end_out(soap)
	 || soap_envelope_end_out(soap)
	 || soap_end_send(soap))
		return soap->error;
	return soap_closesock(soap);
}

SOAP_FMAC5 int SOAP_FMAC6 soap_serve_ns__SaveDeviceStatusLog(struct soap *soap)
{	struct ns__SaveDeviceStatusLog soap_tmp_ns__SaveDeviceStatusLog;
	ns__ErrorCode responseErrorInfo;
	responseErrorInfo.soap_default(soap);
	soap_default_ns__SaveDeviceStatusLog(soap, &soap_tmp_ns__SaveDeviceStatusLog);
	if (!soap_get_ns__SaveDeviceStatusLog(soap, &soap_tmp_ns__SaveDeviceStatusLog, "ns:SaveDeviceStatusLog", NULL))
		return soap->error;
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
		return soap->error;
	soap->error = ns__SaveDeviceStatusLog(soap, soap_tmp_ns__SaveDeviceStatusLog.requestUserInfo, soap_tmp_ns__SaveDeviceStatusLog.requestStatusLogInfo, responseErrorInfo);
	if (soap->error)
		return soap->error;
	soap->encodingStyle = NULL;
	soap_serializeheader(soap);
	responseErrorInfo.soap_serialize(soap);
	if (soap_begin_count(soap))
		return soap->error;
	if (soap->mode & SOAP_IO_LENGTH)
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || responseErrorInfo.soap_put(soap, "ns:ErrorCode", "")
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	};
	if (soap_end_count(soap)
	 || soap_response(soap, SOAP_OK)
	 || soap_envelope_begin_out(soap)
	 || soap_putheader(soap)
	 || soap_body_begin_out(soap)
	 || responseErrorInfo.soap_put(soap, "ns:ErrorCode", "")
	 || soap_body_end_out(soap)
	 || soap_envelope_end_out(soap)
	 || soap_end_send(soap))
		return soap->error;
	return soap_closesock(soap);
}

SOAP_FMAC5 int SOAP_FMAC6 soap_serve_ns__GetDeviceStatusLog(struct soap *soap)
{	struct ns__GetDeviceStatusLog soap_tmp_ns__GetDeviceStatusLog;
	ns__DeviceStatusLogList responseStatusLogList;
	responseStatusLogList.soap_default(soap);
	soap_default_ns__GetDeviceStatusLog(soap, &soap_tmp_ns__GetDeviceStatusLog);
	if (!soap_get_ns__GetDeviceStatusLog(soap, &soap_tmp_ns__GetDeviceStatusLog, "ns:GetDeviceStatusLog", NULL))
		return soap->error;
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
		return soap->error;
	soap->error = ns__GetDeviceStatusLog(soap, soap_tmp_ns__GetDeviceStatusLog.requestUserInfo, soap_tmp_ns__GetDeviceStatusLog.requestStatusLogInfo, responseStatusLogList);
	if (soap->error)
		return soap->error;
	soap->encodingStyle = NULL;
	soap_serializeheader(soap);
	responseStatusLogList.soap_serialize(soap);
	if (soap_begin_count(soap))
		return soap->error;
	if (soap->mode & SOAP_IO_LENGTH)
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || responseStatusLogList.soap_put(soap, "ns:DeviceStatusLogList", "")
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	};
	if (soap_end_count(soap)
	 || soap_response(soap, SOAP_OK)
	 || soap_envelope_begin_out(soap)
	 || soap_putheader(soap)
	 || soap_body_begin_out(soap)
	 || responseStatusLogList.soap_put(soap, "ns:DeviceStatusLogList", "")
	 || soap_body_end_out(soap)
	 || soap_envelope_end_out(soap)
	 || soap_end_send(soap))
		return soap->error;
	return soap_closesock(soap);
}

SOAP_FMAC5 int SOAP_FMAC6 soap_serve_ns__GetServerOptions(struct soap *soap)
{	struct ns__GetServerOptions soap_tmp_ns__GetServerOptions;
	ns__ServerOptions serverOptions;
	serverOptions.soap_default(soap);
	soap_default_ns__GetServerOptions(soap, &soap_tmp_ns__GetServerOptions);
	if (!soap_get_ns__GetServerOptions(soap, &soap_tmp_ns__GetServerOptions, "ns:GetServerOptions", NULL))
		return soap->error;
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
		return soap->error;
	soap->error = ns__GetServerOptions(soap, soap_tmp_ns__GetServerOptions.requestUserInfo, serverOptions);
	if (soap->error)
		return soap->error;
	soap->encodingStyle = NULL;
	soap_serializeheader(soap);
	serverOptions.soap_serialize(soap);
	if (soap_begin_count(soap))
		return soap->error;
	if (soap->mode & SOAP_IO_LENGTH)
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || serverOptions.soap_put(soap, "ns:ServerOptions", "")
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	};
	if (soap_end_count(soap)
	 || soap_response(soap, SOAP_OK)
	 || soap_envelope_begin_out(soap)
	 || soap_putheader(soap)
	 || soap_body_begin_out(soap)
	 || serverOptions.soap_put(soap, "ns:ServerOptions", "")
	 || soap_body_end_out(soap)
	 || soap_envelope_end_out(soap)
	 || soap_end_send(soap))
		return soap->error;
	return soap_closesock(soap);
}

SOAP_FMAC5 int SOAP_FMAC6 soap_serve_ns__SetServerOptions(struct soap *soap)
{	struct ns__SetServerOptions soap_tmp_ns__SetServerOptions;
	ns__ErrorCode responseErrorInfo;
	responseErrorInfo.soap_default(soap);
	soap_default_ns__SetServerOptions(soap, &soap_tmp_ns__SetServerOptions);
	if (!soap_get_ns__SetServerOptions(soap, &soap_tmp_ns__SetServerOptions, "ns:SetServerOptions", NULL))
		return soap->error;
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
		return soap->error;
	soap->error = ns__SetServerOptions(soap, soap_tmp_ns__SetServerOptions.requestUserInfo, soap_tmp_ns__SetServerOptions.serverOptions, responseErrorInfo);
	if (soap->error)
		return soap->error;
	soap->encodingStyle = NULL;
	soap_serializeheader(soap);
	responseErrorInfo.soap_serialize(soap);
	if (soap_begin_count(soap))
		return soap->error;
	if (soap->mode & SOAP_IO_LENGTH)
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || responseErrorInfo.soap_put(soap, "ns:ErrorCode", "")
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	};
	if (soap_end_count(soap)
	 || soap_response(soap, SOAP_OK)
	 || soap_envelope_begin_out(soap)
	 || soap_putheader(soap)
	 || soap_body_begin_out(soap)
	 || responseErrorInfo.soap_put(soap, "ns:ErrorCode", "")
	 || soap_body_end_out(soap)
	 || soap_envelope_end_out(soap)
	 || soap_end_send(soap))
		return soap->error;
	return soap_closesock(soap);
}

SOAP_FMAC5 int SOAP_FMAC6 soap_serve_ns__GetCityAndLocations(struct soap *soap)
{	struct ns__GetCityAndLocations soap_tmp_ns__GetCityAndLocations;
	ns__AllCityLocatoins responseCityLocationInfo;
	responseCityLocationInfo.soap_default(soap);
	soap_default_ns__GetCityAndLocations(soap, &soap_tmp_ns__GetCityAndLocations);
	if (!soap_get_ns__GetCityAndLocations(soap, &soap_tmp_ns__GetCityAndLocations, "ns:GetCityAndLocations", NULL))
		return soap->error;
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
		return soap->error;
	soap->error = ns__GetCityAndLocations(soap, soap_tmp_ns__GetCityAndLocations.requestUserInfo, responseCityLocationInfo);
	if (soap->error)
		return soap->error;
	soap->encodingStyle = NULL;
	soap_serializeheader(soap);
	responseCityLocationInfo.soap_serialize(soap);
	if (soap_begin_count(soap))
		return soap->error;
	if (soap->mode & SOAP_IO_LENGTH)
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || responseCityLocationInfo.soap_put(soap, "ns:AllCityLocatoins", "")
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	};
	if (soap_end_count(soap)
	 || soap_response(soap, SOAP_OK)
	 || soap_envelope_begin_out(soap)
	 || soap_putheader(soap)
	 || soap_body_begin_out(soap)
	 || responseCityLocationInfo.soap_put(soap, "ns:AllCityLocatoins", "")
	 || soap_body_end_out(soap)
	 || soap_envelope_end_out(soap)
	 || soap_end_send(soap))
		return soap->error;
	return soap_closesock(soap);
}

SOAP_FMAC5 int SOAP_FMAC6 soap_serve_ns__ChangeCityName(struct soap *soap)
{	struct ns__ChangeCityName soap_tmp_ns__ChangeCityName;
	ns__ErrorCode responseErrorInfo;
	responseErrorInfo.soap_default(soap);
	soap_default_ns__ChangeCityName(soap, &soap_tmp_ns__ChangeCityName);
	if (!soap_get_ns__ChangeCityName(soap, &soap_tmp_ns__ChangeCityName, "ns:ChangeCityName", NULL))
		return soap->error;
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
		return soap->error;
	soap->error = ns__ChangeCityName(soap, soap_tmp_ns__ChangeCityName.requestUserInfo, soap_tmp_ns__ChangeCityName.oldCityName, soap_tmp_ns__ChangeCityName.newCityName, responseErrorInfo);
	if (soap->error)
		return soap->error;
	soap->encodingStyle = NULL;
	soap_serializeheader(soap);
	responseErrorInfo.soap_serialize(soap);
	if (soap_begin_count(soap))
		return soap->error;
	if (soap->mode & SOAP_IO_LENGTH)
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || responseErrorInfo.soap_put(soap, "ns:ErrorCode", "")
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	};
	if (soap_end_count(soap)
	 || soap_response(soap, SOAP_OK)
	 || soap_envelope_begin_out(soap)
	 || soap_putheader(soap)
	 || soap_body_begin_out(soap)
	 || responseErrorInfo.soap_put(soap, "ns:ErrorCode", "")
	 || soap_body_end_out(soap)
	 || soap_envelope_end_out(soap)
	 || soap_end_send(soap))
		return soap->error;
	return soap_closesock(soap);
}

SOAP_FMAC5 int SOAP_FMAC6 soap_serve_ns__ChangeLocationName(struct soap *soap)
{	struct ns__ChangeLocationName soap_tmp_ns__ChangeLocationName;
	ns__ErrorCode responseErrorInfo;
	responseErrorInfo.soap_default(soap);
	soap_default_ns__ChangeLocationName(soap, &soap_tmp_ns__ChangeLocationName);
	if (!soap_get_ns__ChangeLocationName(soap, &soap_tmp_ns__ChangeLocationName, "ns:ChangeLocationName", NULL))
		return soap->error;
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
		return soap->error;
	soap->error = ns__ChangeLocationName(soap, soap_tmp_ns__ChangeLocationName.requestUserInfo, soap_tmp_ns__ChangeLocationName.oldCityName, soap_tmp_ns__ChangeLocationName.newCityName, soap_tmp_ns__ChangeLocationName.oldLocationName, soap_tmp_ns__ChangeLocationName.newLocationName, responseErrorInfo);
	if (soap->error)
		return soap->error;
	soap->encodingStyle = NULL;
	soap_serializeheader(soap);
	responseErrorInfo.soap_serialize(soap);
	if (soap_begin_count(soap))
		return soap->error;
	if (soap->mode & SOAP_IO_LENGTH)
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || responseErrorInfo.soap_put(soap, "ns:ErrorCode", "")
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	};
	if (soap_end_count(soap)
	 || soap_response(soap, SOAP_OK)
	 || soap_envelope_begin_out(soap)
	 || soap_putheader(soap)
	 || soap_body_begin_out(soap)
	 || responseErrorInfo.soap_put(soap, "ns:ErrorCode", "")
	 || soap_body_end_out(soap)
	 || soap_envelope_end_out(soap)
	 || soap_end_send(soap))
		return soap->error;
	return soap_closesock(soap);
}

SOAP_FMAC5 int SOAP_FMAC6 soap_serve_ns__ChangeDeviceName(struct soap *soap)
{	struct ns__ChangeDeviceName soap_tmp_ns__ChangeDeviceName;
	ns__ErrorCode responseErrorInfo;
	responseErrorInfo.soap_default(soap);
	soap_default_ns__ChangeDeviceName(soap, &soap_tmp_ns__ChangeDeviceName);
	if (!soap_get_ns__ChangeDeviceName(soap, &soap_tmp_ns__ChangeDeviceName, "ns:ChangeDeviceName", NULL))
		return soap->error;
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
		return soap->error;
	soap->error = ns__ChangeDeviceName(soap, soap_tmp_ns__ChangeDeviceName.requestUserInfo, soap_tmp_ns__ChangeDeviceName.deviceSerialNumber, soap_tmp_ns__ChangeDeviceName.newCityName, soap_tmp_ns__ChangeDeviceName.newLocationName, soap_tmp_ns__ChangeDeviceName.newDeviceName, responseErrorInfo);
	if (soap->error)
		return soap->error;
	soap->encodingStyle = NULL;
	soap_serializeheader(soap);
	responseErrorInfo.soap_serialize(soap);
	if (soap_begin_count(soap))
		return soap->error;
	if (soap->mode & SOAP_IO_LENGTH)
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || responseErrorInfo.soap_put(soap, "ns:ErrorCode", "")
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	};
	if (soap_end_count(soap)
	 || soap_response(soap, SOAP_OK)
	 || soap_envelope_begin_out(soap)
	 || soap_putheader(soap)
	 || soap_body_begin_out(soap)
	 || responseErrorInfo.soap_put(soap, "ns:ErrorCode", "")
	 || soap_body_end_out(soap)
	 || soap_envelope_end_out(soap)
	 || soap_end_send(soap))
		return soap->error;
	return soap_closesock(soap);
}

SOAP_FMAC5 int SOAP_FMAC6 soap_serve_ns__TestWorld(struct soap *soap)
{	struct ns__TestWorld soap_tmp_ns__TestWorld;
	struct ns__TestWorldResponse soap_tmp_ns__TestWorldResponse;
	soap_default_ns__TestWorldResponse(soap, &soap_tmp_ns__TestWorldResponse);
	soap_default_ns__TestWorld(soap, &soap_tmp_ns__TestWorld);
	if (!soap_get_ns__TestWorld(soap, &soap_tmp_ns__TestWorld, "ns:TestWorld", NULL))
		return soap->error;
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
		return soap->error;
	soap->error = ns__TestWorld(soap, soap_tmp_ns__TestWorld.requestUserInfo, soap_tmp_ns__TestWorldResponse.responseErrorInfo);
	if (soap->error)
		return soap->error;
	soap->encodingStyle = NULL;
	soap_serializeheader(soap);
	soap_serialize_ns__TestWorldResponse(soap, &soap_tmp_ns__TestWorldResponse);
	if (soap_begin_count(soap))
		return soap->error;
	if (soap->mode & SOAP_IO_LENGTH)
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || soap_put_ns__TestWorldResponse(soap, &soap_tmp_ns__TestWorldResponse, "ns:TestWorldResponse", NULL)
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	};
	if (soap_end_count(soap)
	 || soap_response(soap, SOAP_OK)
	 || soap_envelope_begin_out(soap)
	 || soap_putheader(soap)
	 || soap_body_begin_out(soap)
	 || soap_put_ns__TestWorldResponse(soap, &soap_tmp_ns__TestWorldResponse, "ns:TestWorldResponse", NULL)
	 || soap_body_end_out(soap)
	 || soap_envelope_end_out(soap)
	 || soap_end_send(soap))
		return soap->error;
	return soap_closesock(soap);
}

#if defined(__BORLANDC__)
#pragma option pop
#pragma option pop
#endif

/* End of Server_SoapServer.cpp */
